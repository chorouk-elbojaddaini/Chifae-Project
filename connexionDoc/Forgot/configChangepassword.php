...
if (isset($_GET['reset'])) {
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM medecin WHERE code='{$_GET['reset']}'")) > 0) {
        if (isset($_POST['submit'])) {
            ...
            elseif($password === $confirm_password) {
                ...
                $query = mysqli_query($conn, "UPDATE medecin SET motdepasse='{$password}', code='' WHERE code='{$_GET['reset']}'");
