...
function filter_by_date($table,$date,$start,$nb,$field,$search,$conn)
{
    if($date==='cemois')
    {
        $query = mysqli_query($conn , "SELECT * from $table  WHERE MONTH(date) = MONTH(now()) AND YEAR(date) = YEAR(now()) AND $field  LIKE'%$search%'ORDER BY date DESC limit $start,$nb ");
    ...
    if($date==='moisprec')
    {
        $query = mysqli_query($conn , "SELECT * from $table where MONTH( date ) = MONTH( DATE_SUB(CURDATE(),INTERVAL 1 MONTH )) AND YEAR(date) = YEAR( DATE_SUB(CURDATE( ),INTERVAL 1 MONTH ))AND $field  LIKE'%$search%'ORDER BY date DESC limit $start,$nb ");
    ...
    {
        $query = mysqli_query($conn , "SELECT * from $table where YEAR(date)>=YEAR( DATE_SUB(CURDATE(),INTERVAL 5 YEAR )) AND YEAR(date)<YEAR(CURDATE()) AND $field  LIKE'%$search%'ORDER BY date DESC limit $start,$nb ;");
        $query1 = mysqli_query($conn , "SELECT * from $table where YEAR(date)>=YEAR( DATE_SUB(CURDATE(),INTERVAL 5 YEAR )) AND YEAR(date)<YEAR(CURDATE())  AND $field  LIKE'%$search%' ");
    }
    if($date==='tous')
    {
        $query = mysqli_query($conn,"SELECT * FROM $table WHERE  $field  LIKE'%$search%' ORDER BY date DESC limit $start,$nb ;");
